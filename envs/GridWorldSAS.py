...
class Gridworld_SAS(object):
    def __init__(self,
        ...
        self.observation_space = Space(low=np.zeros(2, dtype=np.float32), high=np.ones(2, dtype=np.float32),
                                       dtype=np.float32)
        ...
        self.n_lidar = 0
        self.angles = np.linspace(0, 2 * np.pi, self.n_lidar + 1)[
                      :-1]  # Get 10 lidar directions,(11th and 0th are same)
    ...
    def get_env_state(self):
        return self.curr_state.copy(), 0, self.is_terminal()
